...
PlayerLastRecord AS (
    ...
    WHERE
        ...
        -- [!] 请在这里设定您要统计的日期范围
        AND e.day ${PartDate:date1}
        AND e.proj_dice IS NOT NULL
),

-- 3. 获取每个玩家的最后一条记录的骰子余量
PlayerDiceBalance AS (
    SELECT
        user_id,
        dice_balance
    FROM
...
-- 4. 合并数据：玩家最小 map_level + 骰子余量
PlayerStats AS (
    SELECT
        pml.user_id,
        pml.min_map_level,
        COALESCE(pdb.dice_balance, 0) AS dice_balance
    FROM
        PlayerMinMapLevel pml
    LEFT JOIN
        PlayerDiceBalance pdb
        ON pml.user_id = pdb.user_id
),
...
-- 6. 按聚类后的 map_level 分组，计算骰子余量的分位数分布
SELECT
    map_level_group AS map_level,
    COUNT(user_id) AS user_count,
    -- 骰子余量分位数
    ROUND(PERCENTILE_CONT(0.25) WITHIN GROUP (ORDER BY dice_balance ASC), 0) AS dice_balance_p25,
    ROUND(PERCENTILE_CONT(0.50) WITHIN GROUP (ORDER BY dice_balance ASC), 0) AS dice_balance_p50,
    ROUND(PERCENTILE_CONT(0.75) WITHIN GROUP (ORDER BY dice_balance ASC), 0) AS dice_balance_p75,
    ROUND(PERCENTILE_CONT(0.90) WITHIN GROUP (ORDER BY dice_balance ASC), 0) AS dice_balance_p90
FROM
    LevelClustered
GROUP BY
    map_level_group
