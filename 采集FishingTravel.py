...
logging.basicConfig(
    ...
    handlers=[
        logging.FileHandler('fishing_travel_scrape.log', encoding='utf-8'),
...
def main():
    """主函数"""
    logger.info("="*60)
    logger.info("Fishing Travel 数据采集")
    logger.info("="*60)
    ...
    for game in config['games']:
        if game['name'] == 'Fishing Travel':
            game_config = game
            break
    
    if not game_config:
        logger.error("未找到Fishing Travel配置！")
        return
    ...
    if not app_id:
        logger.error("Fishing Travel的Google Play ID未配置！")
    ...
    logger.info("="*60)
    logger.info(f"\n数据已保存到: {output_path}")
    logger.info(f"\n下一步：运行 '运行FishingTravel筛选.bat' 进行评论筛选")
    logger.info("="*60)
