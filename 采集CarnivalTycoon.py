...
logging.basicConfig(
    ...
    handlers=[
        logging.FileHandler('carnival_tycoon_scrape.log', encoding='utf-8'),
...
def main():
    """主函数"""
    logger.info("="*60)
    logger.info("Carnival Tycoon 数据采集")
    logger.info("="*60)
    ...
    for game in config['games']:
        if game['name'] == 'Carnival Tycoon':
            game_config = game
            break
    
    if not game_config:
        logger.error("未找到Carnival Tycoon配置！")
        return
    ...
    if not app_id:
        logger.error("Carnival Tycoon的Google Play ID未配置！")
    ...
    # 保存数据
    output_path = f"data/raw/{game_name.replace(' ', '_')}_android_{region['name']}_{start_date.strftime('%Y%m')}-{end_date.strftime('%Y%m')}.json"
    ...
    logger.info("="*60)
    logger.info(f"\n数据已保存到: {output_path}")
    logger.info(f"\n下一步：运行 '运行CarnivalTycoon筛选.bat' 进行评论筛选")
    logger.info("="*60)
